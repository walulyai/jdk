...
#include "utilities/intrusiveList.inline.hpp"
...
struct TestIntrusiveListValue : public CHeapObj<mtInternal> {
  ...
  bool is_attached1() const { return entry1(this)->is_linked(); }
  bool is_attached2() const { return entry2(this)->is_linked(); }
...
////////////////////
// push_front(), pop_front(), size(), empty()
// front(), back()
...
TEST_F(IntrusiveListTestWithDisposal, pop) {
  ASSERT_EQ(nvalues, list1.size());
  ASSERT_EQ(values[0], list1.front()->This());
  ASSERT_EQ(values[nvalues - 1], list1.back()->This());

  list1.pop_front();
  EXPECT_EQ(nvalues - 1, list1.size());
  EXPECT_EQ(values[1], list1.front()->This());

  list1.erase_and_dispose(list1.begin(), CollectingDisposer(this));
  EXPECT_EQ(nvalues - 2, list1.size());
  EXPECT_EQ(values[2], list1.front()->This());
  EXPECT_EQ(1u, ndisposed);
  EXPECT_EQ(values[1], disposed[0]);

  list1.pop_back();
  EXPECT_EQ(nvalues - 3, list1.size());
  EXPECT_EQ(values[nvalues - 2], list1.back()->This());

  list1.erase_and_dispose(--list1.end(), CollectingDisposer(this));
  EXPECT_EQ(nvalues - 4, list1.size());
...
TEST_F(IntrusiveListTestWithList1, erase2) {
  ...
  EXPECT_EQ(nvalues - step2, list1.size());
...
TEST_F(IntrusiveListTestWithDisposal, erase2_dispose) {
  ...
  EXPECT_EQ(nvalues - step3, list1.size());
...
TEST_F(IntrusiveListTestSplice, splice_all_front) {
  size_t a_size = list_a.size();
  size_t b_size = list_b.size();
  List1::iterator a_begin = list_a.begin();
  List1::iterator a_end = list_a.end();
  List1::iterator b_begin = list_b.begin();

  list_a.splice(a_begin, list_b);
  EXPECT_EQ(list_a.size(), a_size + b_size);
...
TEST_F(IntrusiveListTestSplice, splice_some_middle) {
  ...
  list_a.splice(a_middle, list_b, b_move_start, b_move_end);
  EXPECT_EQ(list_a.size(), a_size + move_size);
  EXPECT_EQ(list_b.size(), b_size - move_size);
  EXPECT_EQ(a_begin, list_a.cbegin());
  EXPECT_EQ(a_end, list_a.cend());
...
TEST_F(IntrusiveListTestSplice, splice_one_front) {
  ...
  list_a.splice(a_begin, list_b, b_move_start);
  EXPECT_EQ(list_a.size(), a_size + 1);
  EXPECT_EQ(list_b.size(), b_size - 1);
...
TEST_F(IntrusiveListTestSplice, splice_one_back) {
  ...
  list_a.splice(a_end, list_b, b_move_start);
  ++a_penult;
...
TEST_F(IntrusiveListTestSplice, splice_one_in_place) {
  ...
  list_a.splice(a_move_end, list_a, a_move_start);
  EXPECT_EQ(list_a.size(), a_size);
...
TEST_F(IntrusiveListTestSplice, splice_into_const) {
  ...
  size_t a_size = list_a.size();
  size_t b_size = list_b.size();
  clist.splice(clist.end(), list_a);
  clist.splice(clist.end(), list_b);
  EXPECT_EQ(clist.size(), a_size + b_size);
  {
    SCOPED_TRACE("check values");
...
TEST_F(IntrusiveListTestSplice, swap) {
  List1::pointer front_a = list_a.front();
  List1::pointer front_b = list_b.front();
  list_a.swap(list_b);

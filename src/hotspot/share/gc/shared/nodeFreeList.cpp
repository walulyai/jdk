...
NodeFreeList::PendingList::PendingList() :
  _tail(nullptr), _head(nullptr), _count(0) {}

NodeFreeList::PendingList::~PendingList() { }

size_t NodeFreeList::PendingList::add(FreeNode* node) {
...
typename NodeFreeList::NodeList NodeFreeList::PendingList::take_all() {
...
size_t NodeFreeList::PendingList::count() const {
  return  Atomic::load(&_count);
}

NodeFreeList::NodeFreeList(const char* name) :
  _free_count(0),
  _free_list(),
...
void NodeFreeList::delete_list(DeleteFn delete_fn) {
...
NodeFreeList::~NodeFreeList() {
  assert(_free_count == 0, "post-condition");
}

void NodeFreeList::reset() {
  try_transfer_pending();
  _free_list.pop_all();
  _free_count = 0;
}

bool NodeFreeList::flush() {
  return try_transfer_pending();
}

size_t NodeFreeList::free_count() const {
  return Atomic::load(&_free_count);
}

size_t NodeFreeList::pending_count() const {
  uint index = Atomic::load(&_active_pending_list);
  return _pending_lists[index].count();;
}

void* NodeFreeList::get() {
...
void NodeFreeList::release(void* free_node) {
...
bool NodeFreeList::try_transfer_pending() {
...
size_t NodeFreeList::reduce_free_list(size_t remove_goal, DeleteFn delete_fn) {
  try_transfer_pending();
  size_t removed = 0;

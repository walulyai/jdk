...
void G1Policy::init(G1CollectedHeap* g1h, G1CollectionSet* collection_set) {
  ...
  update_num_young_regions_bounds();
}
 ...
 public:
  ...
  bool will_fit(uint num_eden_regions) const {
    if (num_eden_regions >= _base_free_regions) {
      // end condition 1: not enough space for the eden regions
      return false;
    }

    size_t bytes_to_copy = 0;
    const double copy_time_ms = _policy->predict_eden_copy_time_ms(num_eden_regions, &bytes_to_copy);
...
void G1Policy::update_num_young_regions_bounds() {
  ...
  update_num_young_regions_bounds(_analytics->predict_pending_cards(for_young_only_phase),
                                  _analytics->predict_card_rs_length(for_young_only_phase),
                                  _analytics->predict_code_root_rs_length(for_young_only_phase));
}

void G1Policy::update_num_young_regions_bounds(size_t pending_cards, size_t card_rs_length, size_t code_root_rs_length) {
  uint prev_target_num_young_regions = target_num_young_regions();

  uint min_num_young_by_sizer = _young_gen_sizer.min_desired_num_regions();
  uint max_num_young_by_sizer = _young_gen_sizer.max_desired_num_regions();

  if (max_num_young_by_sizer < min_num_young_by_sizer) {
    // This can happen due to races with heap_size_changed() at mutator time.
    // Do not update the desired/target numbers of young regions. They will be
    // updated on the next regular call anyway.
    assert(!SafepointSynchronize::is_at_safepoint(), "must be");
    return;
  }

  uint new_desired_num_young =
          calculate_desired_num_young_regions(pending_cards,
                                              card_rs_length,
                                              code_root_rs_length,
                                              min_num_young_by_sizer,
                                              max_num_young_by_sizer);

  uint new_target_num_young = calculate_target_num_young_regions(new_desired_num_young, min_num_young_by_sizer);

  log_trace(gc, ergo, heap)("Young regions update: pending cards %zu card_rs_length %zu prev target %u desired %u target %u",
                            pending_cards,
                            card_rs_length,
                            prev_target_num_young_regions,
                            new_desired_num_young,
                            new_target_num_young);

  // Write back. This is not an attempt to control visibility order to other threads
  // here; revisions to the number of young regions are best-effort attempts to
  // keep pause time.
  ...
  // early or too late.
  _desired_num_young_regions.store_relaxed(new_desired_num_young);
  _target_num_young_regions.store_relaxed(new_target_num_young);
}

// Calculates desired number of young regions. It is calculated from:
//
...
//
uint G1Policy::calculate_desired_num_young_regions(size_t pending_cards,
  ...
  const uint num_young_regions = _g1h->young_regions_count();
  // This is the absolute minimum number of young regions that we can return. Ensure
  // that we don't go below any user-defined minimum bound. Also, we must have at
  // least one eden region, to ensure progress. But when revising during the ensuing
  // mutator phase we might have already allocated more than either of those, in
  // which case use that.
  uint absolute_min_num_young_regions = MAX3(min_num_young_regions_by_sizer,
                                             num_survivor_regions + 1,
                                             num_young_regions);
  // Calculate the absolute max bounds. After evac failure or when revising the
  // number of young regions we might have exceeded the absolute min or max young
  // region bounds, so adjust the result accordingly.
  uint absolute_max_num_young_regions = MAX2(max_num_young_regions_by_sizer, absolute_min_num_young_regions);

  uint desired_num_eden_regions_by_mmu = 0;
  uint desired_num_eden_regions_by_pause = 0;

  uint desired_num_young_regions = 0;
  if (use_adaptive_num_young_regions()) {
    ...
    uint desired_num_eden_regions = MAX2(desired_num_eden_regions_by_pause,
                                         desired_num_eden_regions_by_mmu);

    desired_num_young_regions = desired_num_eden_regions + num_survivor_regions;
  } else {
    // The user asked for a fixed young gen so we'll fix the young gen
    // whether the next GC is young or mixed.
    desired_num_young_regions = min_num_young_regions_by_sizer;
  }
  // Clamp to absolute min/max after determining the desired number of regions.
  desired_num_young_regions = clamp(desired_num_young_regions, absolute_min_num_young_regions, absolute_max_num_young_regions);

  log_trace(gc, ergo, heap)("Desired young regions %u "
                            "survivor regions %u "
                            "allocated young regions %u "
                            "absolute min young regions %u "
                            "absolute max young regions %u "
                            "desired eden regions by mmu %u "
                            "desired eden regions by pause %u ",
                            desired_num_young_regions, num_survivor_regions,
                            num_young_regions, absolute_min_num_young_regions,
                            absolute_max_num_young_regions, desired_num_eden_regions_by_mmu,
                            desired_num_eden_regions_by_pause);

  assert(desired_num_young_regions >= num_young_regions, "must be");
  return desired_num_young_regions;
}

// Limit the desired (wished) number of young regions by current free regions. If the request
...
uint G1Policy::calculate_target_num_young_regions(uint desired_num_young_regions,
  ...
  if (num_young_regions >= desired_num_young_regions) {
    ...
    log_trace(gc, ergo, heap)("Target young regions: Already used up desired young regions %u allocated young regions %u",
    ...
    // method will return a too small target number of young regions in that case.
    uint reserve_regions = _reserve_regions.load_relaxed();

    uint max_to_eat_into_reserve = MIN2(min_num_young_regions_by_sizer,
                                        (reserve_regions + 1) / 2);

    log_trace(gc, ergo, heap)("Target young regions: "
                              "Common free regions at end of collection %u "
                              "desired young regions %u "
                              "reserve regions %u "
    ...
    uint allocated_num_eden_regions = num_young_regions - num_survivor_regions;

    if (_free_regions_at_end_of_collection <= reserve_regions) {
      // Fully eat (or already eating) into the reserve, hand back at most the minimum number of regions.
      ...
      // above.
      receiving_additional_eden = allocated_num_eden_regions < receiving_eden ?
                                  receiving_eden - allocated_num_eden_regions : 0;
      ...
      assert(free_outside_reserve < desired_num_eden_regions,
             "must be %u %u", free_outside_reserve, desired_num_eden_regions);

      uint receiving_within_reserve = MIN2(desired_num_eden_regions - free_outside_reserve,
                                           max_to_eat_into_reserve);
      ...
      receiving_additional_eden = desired_num_young_regions - num_young_regions;
  ...
  uint target_num_young_regions = num_young_regions + receiving_additional_eden;

  assert(target_num_young_regions >= num_young_regions, "must be");

  log_trace(gc, ergo, heap)("Target young regions: "
                            "target young regions %u "
                            "allocated young regions %u "
                            "received additional eden %u",
                            target_num_young_regions, num_young_regions,
                            receiving_additional_eden);
...
uint G1Policy::calculate_desired_num_eden_regions_before_young_only(double base_time_ms,
  ...
  if (p.will_fit(min_num_eden_regions)) {
    ...
    if (p.will_fit(max_num_eden_regions)) {
      ...
      // The results is min_num_eden_regions which, according to the
...
void G1Policy::revise_target_num_young_regions(size_t pending_cards, size_t card_rs_length, size_t code_root_rs_length) {
  ...
  update_num_young_regions_bounds(pending_cards, card_rs_length, code_root_rs_length);
}
...
bool G1Policy::update_ihop_prediction(double mutator_time_s,
  ...
  if (this_gc_was_young_only && mutator_time_s > min_valid_time) {
    ...
    // restrained by the heap reserve. Using the actual number of regions would
    // make the prediction too small and limit the young gen every time we get to
...
bool G1Policy::use_adaptive_num_young_regions() const {
  return _young_gen_sizer.use_adaptive_sizing();
}

...
G1CollectorState G1Policy::record_young_collection_end(bool concurrent_operation_is_full_mark,
  ...
  if (_g1h->gc_cause() != GCCause::_g1_periodic_collection) {
    ...
    // may complete the concurrent cycle.
    size_t current_non_humongous_allocation = _concurrent_cycle_tracker.non_humongous_allocated_bytes();
    size_t current_peak_extra_humongous_occupancy = _concurrent_cycle_tracker.peak_extra_humongous_occupancy_bytes();
    ...
    if (update_ihop_prediction(app_time_ms / 1000.0, is_young_only_pause)) {
      _ihop_control->report_statistics(_g1h->gc_tracer_stw(),
                                       ...
                                       current_non_humongous_allocation,
                                       current_peak_extra_humongous_occupancy
                                      );
    }
  }

...
bool G1CollectedHeap::attempt_allocation_after_gc(size_t word_size,
  ...
  {
    MutexLocker ml(&_alloc_list_lock, Mutex::_no_safepoint_check_flag);
  ...
  if (request._state == StalledAllocReq::AllocationState::Success) {
    *result = request._result;
    ...
    assert(request._state == StalledAllocReq::AllocationState::Failed, "Sanity check!");
...
bool G1CollectedHeap::upgrade_to_full_collection() {
  ...
  {
    log_debug(gc, alloc) ("Reset called at G1CollectedHeap::upgrade_to_full_collection()");
    reset_satisfied();
  }
...
bool G1CollectedHeap::satisfy_failed_allocations(bool expect_null_mutator_alloc_region) {
  ...
  // FIXME: need to expect_null_mutator_alloc_region for different allocator nodes.
  while(true) {
...
G1CollectedHeap::G1CollectedHeap() :
  CollectedHeap(),
  _alloc_list_lock(Mutex::nosafepoint, "G1 Stalled Allocation List"),

...
void G1CodeBlobClosure::HeapRegionGatheringOopClosure::do_oop_work(T* p) {
  ...
  if (!CompressedOops::is_null(oop_or_narrowoop)) {
    ...
    assert(!_g1h->is_in_cset(o) || hr->rem_set()->code_roots_list_contains(_nm) || (hr->is_young()), "if o still in collection set then evacuation failed and nm must already be in the remset");

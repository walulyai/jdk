...
void G1HeapTransition::print() {
  ...
  size_t target_num_eden_after_gc = _g1_heap->policy()->target_num_young_regions() - after._num_survivor_regions;
  size_t max_num_survivor_before_gc = _g1_heap->policy()->max_survivor_regions();
  ...
  log_regions("Eden", _before._num_eden_regions, after._num_eden_regions, target_num_eden_after_gc,
              _before._num_eden_regions_per_node, after._num_eden_regions_per_node);
  log_trace(gc, heap)(" Used: 0K, Waste: 0K");

  log_regions("Survivor", _before._num_survivor_regions, after._num_survivor_regions, max_num_survivor_before_gc,
              _before._num_survivor_regions_per_node, after._num_survivor_regions_per_node);
  log_trace(gc, heap)(" Used: %zuK, Waste: %zuK",
                      usage._survivor_used / K, ((after._num_survivor_regions * G1HeapRegion::GrainBytes) - usage._survivor_used) / K);

  log_info(gc, heap)("Old regions: %zu->%zu",
                     _before._num_old_regions, after._num_old_regions);
  log_trace(gc, heap)(" Used: %zuK, Waste: %zuK",
                      usage._old_used / K, ((after._num_old_regions * G1HeapRegion::GrainBytes) - usage._old_used) / K);

  log_info(gc, heap)("Humongous regions: %zu->%zu",
                     _before._num_humongous_regions, after._num_humongous_regions);
  log_trace(gc, heap)(" Used: %zuK, Waste: %zuK",
                      usage._humongous_used / K, ((after._num_humongous_regions * G1HeapRegion::GrainBytes) - usage._humongous_used) / K);

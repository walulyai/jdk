...
void G1CMBitMapHR::initialize(uint index, G1RegionToSpaceMapper* _bitmap_mapper) {
...
void G1CMBitMap::uncommit_regions(uint start_idx, size_t num_regions) {
  for (uint region_idx = start_idx; region_idx < start_idx + num_regions; region_idx++ ){
    if (_region_livemaps[region_idx].is_initialized()) {
      ...
      _region_livemaps[region_idx].uncommit();
    }
  }
}
...
void G1CMBitMapHR::set(uint index, G1RegionToSpaceMapper* _bitmap_mapper) {
  if (!is_initialized()) {

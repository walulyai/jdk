...
size_t G1IHOPControl::adjusted_target_occupancy() const {
  assert(_is_adaptive, "precondition");
...
// Calculates the old gen threshold for triggering a concurrent marking cycle.
size_t G1IHOPControl::old_gen_threshold_for_conc_mark_start() {
  ...
  // Calculate the old-gen threshold.
  return predicted_needed < target_occupancy
         ? target_occupancy - predicted_needed
         : 0;
}

void G1IHOPControl::print_log(size_t non_young_occupancy) {
  assert(_target_occupancy > 0, "Target occupancy still not updated yet.");
  size_t cur_conc_mark_start_threshold = old_gen_threshold_for_conc_mark_start();
  log_debug(gc, ihop)("Basic information (value update), threshold: %zuMB (%1.2f%%), target occupancy: %zuMB, non-young occupancy: %zuMB (%1.2f%%), "
                      "recent allocation size: %zuMB, recent allocation duration: %1.2fms, recent old gen allocation rate: %1.2fMB/s, recent marking phase length: %1.2fms",
                      cur_conc_mark_start_threshold / M,
                      ...
                      _old_gen_alloc_tracker->last_period_old_gen_bytes() / M,
                      _last_allocation_time_s * 1000.0,
                      _last_allocation_time_s > 0.0 ? _old_gen_alloc_tracker->last_period_old_gen_bytes() / (M * _last_allocation_time_s) : 0.0,
  ...
  size_t actual_threshold = adjusted_target_occupancy();
  log_debug(gc, ihop)("Adaptive IHOP information (value update), threshold: %zuMB (%1.2f), internal target occupancy: %zuMB, "
                      "non-young occupancy: %zuMB, additional buffer size: %zuB, predicted old gen allocation rate: %1.2fMB/s, "

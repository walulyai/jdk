...
class G1Policy: public CHeapObj<mtGC> {
  ...
  // Desired number of young regions without taking actually available free
...
public:
  ...
  // Predict other time for num young regions.
  ...
  // Predict copying live data time for num eden regions. Return the predict bytes if
...
private:
  ...
  // Updates the internal young gen maximum and target number of regions.
  ...
  void update_num_young_regions_bounds();
  void update_num_young_regions_bounds(size_t pending_cards, size_t card_rs_length, size_t code_root_rs_length);
  ...
  uint calculate_target_num_young_regions(uint desired_num_young_regions, uint min_num_young_regions_by_sizer) const;
...
public:
  ...
  // Check the current value of the young regions RSet length and

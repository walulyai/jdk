...
void G1Analytics::update_gc_time_ratios(double end_time_sec, double pause_time_ms, bool is_user_requested_gc) {
  ...
  // User-triggered collections may occur independently of normal GC pressure
  // or mutator allocation behavior, so including their pause times would distort
  // the GC time ratios used for heap sizing.
